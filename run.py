...
def MMSA_run(
    ...
    model_save_dir: str = Path().home() / "autodl-tmp" / "exp_mmsa" / "outputs" / "saved_models",
    res_save_dir: str = Path().home() / "autodl-tmp" / "exp_mmsa" / "outputs" / "results",
    log_dir: str = Path().home() / "autodl-tmp" / "exp_mmsa" / "outputs" / "logs",
):
    ...
    if config_file is not None:
        ...
        if is_tune:
            ...
            config_file = Path(__file__).parent / "config" / "config_regression_ple.json"
    if not config_file.is_file():
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), config_file)
    if model_save_dir is None: # use default model save dir
        model_save_dir = Path.home() / "autodl-tmp" / "exp_mmsa" / "outputs" / "saved_models"
    Path(model_save_dir).mkdir(parents=True, exist_ok=True)
    if res_save_dir is None: # use default result save dir
        res_save_dir = Path.home() / "autodl-tmp" / "exp_mmsa" / "outputs" / "results"
    Path(res_save_dir).mkdir(parents=True, exist_ok=True)
    if log_dir is None: # use default log save dir
        log_dir = Path.home() / "autodl-tmp" / "exp_mmsa" / "outputs" / "logs"
